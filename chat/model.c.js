    ...
    try {
        const chat = await db('message as m')
            .select('c.chat_id',
                    'm.msg_id',
                    'm.message',
                    'u1.username as user1_id', 
                    'u1.username as user1_username', 
                    'u2.username as user2_id', 
...
export const _sendMessage = async({chat_id, sender_id, getmsg_id,message}) => {
    try {
        const chat = await db('message')
        .insert({chat_id, sender_id,getmsg_id, message}, [
            'chat_id',
            'sender_id',
            "getmsg_id",
            'message'
